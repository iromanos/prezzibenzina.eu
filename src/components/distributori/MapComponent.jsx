...
export default function MapComponent({distributori, comuneData}) {
    ...
    // 1. Filtriamo coordinate valide e assicuriamo l'unicità per evitare collisioni di chiavi (key)
    const validDistributori = useMemo(() => {
        if (!distributori) return [];
        const seen = new Set();
        return distributori.filter(d => {
            if (!d.latitudine || !d.longitudine || d.latitudine === 0 || d.longitudine === 0) return false;
            const isDuplicate = seen.has(d.id_impianto);
            seen.add(d.id_impianto);
            return !isDuplicate;
        });
    }, [distributori]);
        ...
        <div className="rounded shadow-sm overflow-hidden border pb-map-canvas-wrapper"
            ...
            >
                ...
                {clusters.map((cluster, index) => {
                    ...
                    if (isCluster) {
                        // Usiamo l'ID del cluster o l'indice come fallback estremo
                        const clusterId = cluster.id || `cluster-${index}`;
                        return (
                            <ClusterMarker
                                key={clusterId}
                                cluster={cluster}
                    ...
                    // Per le stazioni singole, combiniamo l'ID impianto con l'indice per sicurezza
                    return (
                        <StationMarker
                            key={`station-${station.id_impianto}-${index}`}
                            d={station}
                            URI_IMAGE={URI_IMAGE}
                        />
                    );
